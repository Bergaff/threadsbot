...
export class Bot {
 ...
 private async message(m:TgMessage){if(!m.from)return;const uid=m.from.id,cid=m.chat.id,raw=(m.text||m.caption||"").trim();if(await this.db.isBanned(uid))return;this.tg.sendChatAction(cid,"typing").catch(()=>{});
  ...
  if(raw.startsWith("/")){const [command]=raw.split(/\s+/);switch(command.split("@")[0]){case"/start":return this.start(m);case"/language":return this.language(m);case"/help":return this.help(m);case"/terms":return this.terms(m);case"/subscribe":return this.subscribe(m);case"/status":return this.status(m);case"/web":return this.webLink(m);case"/track":return this.track(m);case"/untrack":return this.untrack(m);case"/tracks":return this.tracksList(m);case"/support":return this.support(m);case"/admin":return this.admin(m);case"/report":return this.report(m);case"/ban":return this.ban(m,true);case"/unban":return this.ban(m,false);case"/accounts":return this.accountsList(m);case"/account_check":return this.accountCheck(m);case"/account_del":return this.accountDelete(m);case"/account_export":return this.accountExport(m);case"/diag":return this.diag(m);default:return}}
 ...
 private async webLink(m:TgMessage){const uid=m.from!.id,cid=m.chat.id,lang=await this.lang(uid),sub=await this.db.subscription(uid);const token=await createAuthToken(uid,this.env.WEBHOOK_SECRET);const link=`/?auth=${token}`;if(sub?.active){await this.buttons(cid,`⭐ <b>${lang==="en"?"Web Viewer: Ad-Free Premium":"Веб-зеркало: Премиум без рекламы"}</b>\n\n${lang==="en"?"Your subscription is active until":"Ваша подписка активна до"} ${String(sub.expires_at).slice(0,10)} (${sub.days_left} ${text("days",lang)}).\n\n${lang==="en"?"Open the website with this auth token to disable all ads:":"Откройте сайт с этим токеном для отключения рекламы:"}\n\n<code>?auth=${token}</code>`,kb([[{text:lang==="en"?"🌐 Open site (no ads)":"🌐 Открыть сайт без рекламы",url:`https://threads-reader.workers.dev/?auth=${token}`}]]));}else{await this.buttons(cid,`🌐 <b>${lang==="en"?"Web Viewer Sync":"Синхронизация с веб-зеркалом"}</b>\n\n${lang==="en"?"Your personal auth token for the site:":"Ваш токен для авторизации на сайте:"}\n<code>?auth=${token}</code>\n\n${lang==="en"?"Subscribe via /subscribe to get unlimited bot access AND disable all ads on the website!":"Оформите подписку через /subscribe, чтобы снять лимиты в боте И отключить рекламу на сайте!"}`,kb([[{text:lang==="en"?"💳 Subscribe":"💳 Оформить подписку",callback_data:"sub:choose"}],[{text:lang==="en"?"🌐 Open website":"🌐 Открыть сайт",url:`https://threads-reader.workers.dev/?auth=${token}`}]]));}}
 ...
 private async choice(cb:CallbackQuery){const [mode,username,pageRaw]=cb.data!.split(":"),page=Number(pageRaw),uid=cb.from.id,cid=cb.message!.chat.id,lang=await this.lang(uid);if(page===0){const access=await this.access(uid);if(!access.ok){await this.buttons(cid,"🔒",kb([[{text:text("subscribe_btn",lang),callback_data:"sub:choose"}]]));return}const limited=await this.db.rateLimit(uid);if(limited){await this.tg.editText(cid,cb.message!.message_id,`⏳ ${limited}`);return}}if(!(await this.acquireLock(cid,lang)))return;await this.tg.editMarkup(cid,cb.message!.message_id).catch(()=>{});const loading=await this.tg.sendMessage(cid,`⏳ @${esc(username)}...`);try{let posts=mode==="text"?await this.db.cache<Post[]>(username,mode):null,status:"ok"|string="ok",fetchError="";if(!posts){if(page===0)await Promise.all([this.db.logRequest(uid,username),this.db.logEvent(uid,"search",username),this.db.logEvent(uid,"request",`${mode}:${username}`)]);const fetched=await fetchPosts(this.env,username,mode as "text"|"img",20);posts=fetched.data;status=fetched.status;fetchError=fetched.error||"";if(posts&&status==="ok"&&mode==="text"){await this.db.setCache(username,mode,posts);await this.db.setCache(username,"web_profile",{profile:{username,displayName:username,bio:"",avatar:"",followers:"",verified:false},posts});}if(posts&&status==="ok"&&page===0&&!await this.db.hasSubscription(uid)&&!isAdmin(this.env,uid))await this.db.logEvent(uid,"free_request",username)}await this.tg.deleteMessage(cid,loading.message_id).catch(()=>{});if(status==="all_dead"){await this.alert(`Все аккаунты недоступны! @${username}`);await this.tg.sendMessage(cid,text("all_dead",lang));return}if(status==="browser_busy"){await this.tg.sendMessage(cid,"⏳ Browser Run занят. Аккаунты не отключены; повторите запрос через 30 секунд.");return}if(status==="service_error"){await this.tg.sendMessage(cid,`⚠️ Браузер/Threads: <code>${esc((fetchError||"unknown").slice(0,300))}</code>`);return}if(status==="user_not_found"){await this.tg.sendMessage(cid,text("user_not_found",lang,{username}));return}if(!posts?.length){await this.tg.sendMessage(cid,text("no_posts",lang));return}await this.db.setState(cid,"last_username",username);const start=page*5,current=posts.slice(start,start+5),end=start+current.length;if(!current.length){await this.tg.sendMessage(cid,text("no_more",lang));return}if(page===0)await this.tg.sendMessage(cid,text("posts_found",lang,{username,count:posts.length}));for(let i=0;i<current.length;i++){const p=current[i];if(mode==="text"){let indicator=p.has_image&&p.has_video?` (📷🎥 ${text("photo",lang)}+${text("video",lang)})`:p.has_image?` (📷 ${text("photo",lang)})`:p.has_video?` (🎥 ${text("video",lang)})`:"";await this.tg.sendMessage(cid,`<b>${start+i+1}.</b> @${esc(username)}\n${esc(p.text.slice(0,3900))}${indicator}`)}else if(p.image)await this.tg.sendPhoto(cid,p.image,`${start+i+1}. @${username}`)}const more=posts.length>end,rows:any[][]=[];if(more)rows.push([{text:`➡️ ${text("more",lang)} (${end+1}–${Math.min(end+5,posts.length)})`,callback_data:`${mode}:${username}:${page+1}`}]);rows.push([{text:mode==="text"?text("screens_btn",lang):text("text_btn",lang),callback_data:`${mode==="text"?"img":"text"}:${username}:0`}]);await this.buttons(cid,`✅ <b>${start+1}–${end} ${text("of",lang)} ${posts.length}</b>\n${more?text("more_available",lang):`📭 ${text("all",lang)}`}\n${text("reply_for_comments",lang)}\n${text("send_other",lang)}`,kb(rows))}catch(error){await this.tg.deleteMessage(cid,loading.message_id).catch(()=>{});await this.alert(`Bot error: ${error}`);await this.tg.sendMessage(cid,`❌ <code>${esc(String(error).slice(0,300))}</code>`)}finally{await this.db.clearState(cid,"fetch_lock")}}
 private async replyComments(m:TgMessage){const uid=m.from!.id,cid=m.chat.id,lang=await this.lang(uid),replied=m.reply_to_message!,rawReplied=(replied.text||replied.caption||""),tagged=rawReplied.match(/^(\d+)\.\s*@([A-Za-z0-9._]+)/),match=rawReplied.match(/^(?:📷|🎥)?\s*(?:<b>)?(\d+)\./),fallback=(m.text||"").match(/^(\d+)$/),number=Number(tagged?.[1]||match?.[1]||fallback?.[1]);if(!number)return;const username=tagged?.[2]||await this.db.state(cid,"last_username");if(!username){await this.tg.sendMessage(cid,"❌ Send a username first.");return}const access=await this.access(uid);if(!access.ok){await this.db.logEvent(uid,"free_exhausted",access.kind);await this.buttons(cid,access.note,kb([[{text:text("subscribe_btn",lang),callback_data:"sub:choose"}]]));return}const limited=await this.db.rateLimit(uid);if(limited){await this.tg.sendMessage(cid,`⏳ ${limited}`);return}if(!(await this.acquireLock(cid,lang)))return;const loading=await this.tg.sendMessage(cid,"💬 Загружаю комментарии...");try{const found=await fetchComments(this.env,username,number-1,20);await this.tg.deleteMessage(cid,loading.message_id).catch(()=>{});if(found.status==="all_dead"){await this.tg.sendMessage(cid,text("all_dead",lang));return}if(found.status==="browser_busy"){await this.tg.sendMessage(cid,"⏳ Browser Run занят. Аккаунты не отключены; повторите через 30 секунд.");return}if(found.status==="service_error"){await this.tg.sendMessage(cid,`⚠️ Браузер/Threads: <code>${esc(String(found.error||"unknown").slice(0,300))}</code>`);return}if(found.status==="post_not_found"||found.status==="user_not_found"){await this.tg.sendMessage(cid,"❌ Post not found.");return}if(!found.data?.length){await this.tg.sendMessage(cid,"😔 No comments.");return}await Promise.all([this.db.logRequest(uid,`${username}/cmt/${number}`),this.db.logEvent(uid,"comments_request",username),this.db.logEvent(uid,"request",`comments:${username}`)]);if(!await this.db.hasSubscription(uid)&&!isAdmin(this.env,uid))await this.db.logEvent(uid,"free_request",`cmt:${username}/${number}`);const key=`${username}_cmt_${number-1}`;await this.db.setCache(key,"comments",found.data.map(c=>({text:c.text,author:c.author})));await this.tg.sendMessage(cid,`💬 <b>@${esc(username)}</b> (${found.data.length})`);await this.sendComments(cid,found.data.slice(0,5),0);if(found.data.length>5)await this.buttons(cid,`✅ 1–5 ${text("of",lang)} ${found.data.length}`,kb([[{text:`➡️ ${text("more",lang)} (6–${Math.min(10,found.data.length)})`,callback_data:`cmt:${username}:${number-1}:1`}]]));else await this.tg.sendMessage(cid,`✅ ${text("all",lang)} ${found.data.length}.`)}catch(error){await this.tg.deleteMessage(cid,loading.message_id).catch(()=>{});await this.alert(`Bot error: ${error}`);await this.tg.sendMessage(cid,`❌ <code>${esc(String(error).slice(0,300))}</code>`)}finally{await this.db.clearState(cid,"fetch_lock")}}
 ...
 private async diag(m:TgMessage){
  if(!isAdmin(this.env,m.from!.id))return;
  const counts=await this.db.accountCounts();
  const queue=Boolean(this.env.UPDATES);
  const browser=Boolean(this.env.BROWSER);
  const version=this.env.VERSION||"unknown";
  const paidHint=(!queue||!browser)
